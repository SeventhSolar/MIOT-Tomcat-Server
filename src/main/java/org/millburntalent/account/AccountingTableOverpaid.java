...
@WebServlet("/account/accounting_table_overpaid")
public class AccountingTableOverpaid extends MiotServlet {
	private static final long serialVersionUID = 1L;
	
	

	public AccountingTableOverpaid() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			System.out.println("Result set not properly handled at: AccountingTableOverpaid");
			e.printStackTrace();
		} finally {
			connection.close();
			
			request.getRequestDispatcher("/WEB-INF/header.jsp").include(request, response);
			request.getRequestDispatcher("/WEB-INF/account/accounting_table_overpaid.jsp").include(request, response);
