...
@WebServlet("/account/accounting_table_unpaid")
public class AccountingTableUnpaid extends MiotServlet {
	private static final long serialVersionUID = 1L;
	
	

	public AccountingTableUnpaid() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			System.out.println("Result set not properly handled at: AccountingTableUnpaid");
			e.printStackTrace();
		} finally {
			connection.close();
			
			request.getRequestDispatcher("/WEB-INF/header.jsp").include(request, response);
			request.getRequestDispatcher("/WEB-INF/account/accounting_table_unpaid.jsp").include(request, response);
